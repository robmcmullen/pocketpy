...
for entry in os.listdir('3rd/struct/include'):
    loc = f"../../3rd/struct/include/{entry}"
    headers[loc] = Header(loc)
...
def merge_c_files():
    ...
    multi_dirs = list(os.walk('src/'))
    multi_dirs.extend(list(os.walk('3rd/struct/src/')))
    for root, _, files in multi_dirs:
        for file in files:
            if file.endswith('.c'):
...
shutil.copy("Makefile.amalgamated", "amalgamated/Makefile")
